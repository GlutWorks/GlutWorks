...
function ENT:DecreaseSize()
	if (self.curState == 4) then
		ix.item.Spawn("copper_ore", self:GetPos())
		self:Remove()
	end
	self.curHealth = 200
	...
	ix.item.Spawn("copper_ore", Vector(self:DetermineOutputPlacement()))
	ix.item.Spawn("copper_ore", Vector(self:DetermineOutputPlacement()))
...
if (SERVER) then
	function ENT:SpawnFunction(client, trace)
		local orenode = ents.Create("ix_orenode_copper")
...
if (CLIENT) then
	...
	function ENT:OnPopulateEntityInfo(tooltip)
		...
		name:SetImportant()
		name:SetText("Copper Ore")
		name:SetMaxWidth(math.max(name:GetMaxWidth(), ScrW() * 0.5))
		name:SizeToContents()
	
		local description = tooltip:AddRow("description")
		description:SetText("A node of copper ore.")
